...
LOC_NEWSPAPERS = {
    # PRIMÄR:
    "sn84026749": ("The Sun", "The Sun (New York)", 20),
    # BACKUP (nur wenn Washington Post + The Sun leer):
...
def build_source_registry(date_str):
    ...
    # Deutsche Zeitungen (DDB) – alle möglichen Titel
    ...
    for name in trove_papers.keys():
        registry[name] = {
            "newspaper": name,
            "url": f"https://trove.nla.gov.au/newspaper/",
    ...
    for name in DELPHER_NEWSPAPERS.keys():
        registry[name] = {
            "newspaper": name,
            "url": f"https://www.delpher.nl/nl/kranten",
    ...
    # El Sol (BNE – Biblioteca Nacional de España)
    registry["El Sol"] = {
        "newspaper": "El Sol (Madrid)",
        "url": f"https://hemerotecadigital.bne.es/hd/es/results?parent=0312db23-f8f4-4c11-98ef-fcaa99442e30",
...
def run_download(date_str):
    ...
    if os.path.exists(corpus_file):
        ...
        for name, text in corpus.items():
            ...
            elif name in ("The Sun", "Evening Star", "Washington Post"):
                stats["us_papers"] += 1
    ...
    # 3. US-Zeitungen (Library of Congress – nur The Sun primär)
    print("\n--- US-Zeitungen (Library of Congress: The Sun) ---")
    ...
    delpher_results = fetch_all_delpher(date_str)
    ...
    print("\n--- Australische Zeitungen (Trove) ---")
    trove_results = fetch_trove_newspapers(date_str)
    stats["au_papers"] = 0
    stats["au_chars"] = 0
...
def main():
    ...
    # Stufe 0: Download
    corpus, stats = run_download(TARGET_DATE)
    ...
    # Quellen-Registry aufbauen (dynamisch aus Konfiguration + Datum)
    source_registry = build_source_registry(TARGET_DATE)
